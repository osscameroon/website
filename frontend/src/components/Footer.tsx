...
export function Footer() {
  return <footer className="bg-navy text-white">
    <div className="mx-auto max-w-container px-[clamp(16px,4vw,24px)] py-[clamp(48px,7vw,78px)]">
      <div className="mb-8 flex items-center gap-3"><LogoMark footer className="h-11 w-auto"/><span className="text-[25px] font-bold tracking-[-.02em]">OSS Cameroon</span></div>
      <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,180px),1fr))] gap-[clamp(28px,4vw,48px)]">
        ...
        <FooterCol title="Liens utiles"><Link href="/shop">Shop</Link><a href="https://blog.osscameroon.com" target="_blank" rel="noopener noreferrer">Blog</a></FooterCol>
        <div><h4 className="mb-1 text-base font-bold">Nous contacter</h4><div className="mb-[22px] h-[3px] w-[26px] bg-blue"/><a href="mailto:contact@osscameroon.com" className="text-[14.5px] text-muted-light-3">contact@osscameroon.com</a><div className="mt-[22px] flex gap-3"><Social label="Twitter" href="https://twitter.com/osscameroon"><TwitterIcon className="h-[19px] w-[19px]"/></Social><Social label="GitHub" href="https://github.com/osscameroon"><GitHubIcon className="h-[19px] w-[19px]"/></Social><Social label="Telegram" href="https://t.me/+UpKZh_KXTaTx7JD7"><TelegramIcon className="h-[19px] w-[19px]"/></Social></div></div>
      </div>
      <div className="my-[clamp(32px,5vw,52px)] h-px bg-white/15"/>
      <div className="flex flex-wrap items-center justify-between gap-5"><div className="flex flex-wrap items-center gap-6 text-sm text-muted-light-3"><span>© 2023 OSS Cameroon, All rights reserved.</span><span className="text-muted-light-1">•</span><Link href="#">Privacy policy</Link><Link href="#">Terms of use</Link></div></div>
    </div>
  </footer>;
}
